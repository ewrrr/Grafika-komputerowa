...
/**
 * Create a model of a heptagonal pyramid (7-sided base), matching the
 * pyramid used in earlier labs.  The axis is the z-axis, the base lies
 * in the plane z = -height/2, and the apex is at (0,0,height/2).
 * Each face has its own vertices so lighting and texture coordinates
 * are correct.
 */
function pyramid(size) {
   ...
   function addTri(p0, p1, p2, n, uv0, uv1, uv2) {
      var start = coords.length / 3;
      coords.push(p0[0], p0[1], p0[2], p1[0], p1[1], p1[2], p2[0], p2[1], p2[2]);
      for (var i = 0; i < 3; i++) {
         normals.push(n[0], n[1], n[2]);
      }
      texCoords.push(uv0[0], uv0[1], uv1[0], uv1[1], uv2[0], uv2[1]);
   ...
   for (i = 0; i < slices; i++) {
      ...
      base.push([radius * Math.cos(a), radius * Math.sin(a), baseZ]);
   }

   for (i = 0; i < slices; i++) {
      var p1 = base[i];
      var p2 = base[(i + 1) % slices];
      addTri(apex, p1, p2, faceNormal(apex, p1, p2), [0.5, 1], [0, 0], [1, 0]);
   }

   var center = [0, 0, baseZ];
   var baseN = [0, 0, -1];
   for (i = 0; i < slices; i++) {
